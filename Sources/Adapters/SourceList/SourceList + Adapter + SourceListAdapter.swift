...
extension SourceList.Adapter: SourceListAdapter {
	...
	public func setupFocus(_ identifier: AnyHashable) {
		guard let outlineItem = items[identifier] else {
			return
		}
	...
	public func scrollTo(_ identifier: AnyHashable, withAnimation: Bool = true) {
		guard let outlineItem = items[identifier] else {
			return
		}
		let row = outlineView.row(forItem: outlineItem)
	...
	public func select(identifiers: [AnyHashable], byExtendingSelection extendSelection: Bool = false) {
	...
	public func expand(_ identifier: AnyHashable, withAnimation: Bool = true, expandChildren: Bool = false) {
		guard let outlineItem = items[identifier] else {
			return
		}
		if withAnimation {
			...
			outlineView.expandItem(outlineItem, expandChildren: expandChildren)
		}
	}
}
